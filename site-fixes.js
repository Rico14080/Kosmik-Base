...
function init(){syncBag();bindNoTicket();const observer=new MutationObserver(()=>{syncBag();bindNoTicket()});observer.observe(document.body,{subtree:true,childList:true});window.addEventListener('storage',syncBag);window.addEventListener('pageshow',syncBag)}

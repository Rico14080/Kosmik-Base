...
function renderPublicContent(){const c=getContent(), text=c.siteText||defaultContent.siteText;
  ...
  if(pageKey){const p=c.pages[pageKey]||{};const heading=mainPage.querySelector('.page-heading');if(heading){const e=heading.querySelector('.eyebrow'),h=heading.querySelector('h1'),n=heading.querySelector('.heading-note');if(e){let ey=p.eyebrow||'';if(pageKey==='contact')ey=ey.replace(/[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}/g,'message channel');e.textContent=ey;}if(h)h.innerHTML=`${escapeHtml(p.titleLineOne||'')}<br><em>${escapeHtml(p.titleLineTwo||'')}</em>`;if(n)n.textContent=p.note||'';}}
...
function renderLinksAndLegal(){
  ...
  document.querySelectorAll('[data-contact-socials]').forEach(nav=>{nav.replaceChildren(...links.map(item=>{const a=document.createElement('a');a.href=item.url;a.target='_blank';a.rel='noopener noreferrer';a.textContent=item.label;return a}))});

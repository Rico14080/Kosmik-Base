...
function safeHttpsUrl(v=''){try{const u=new URL(String(v).trim());return u.protocol==='https:'&&!u.username&&!u.password?u.href:''}catch{return ''}}
...
function bindImageFallbacks(){document.querySelectorAll('img').forEach(img=>{if(img.dataset.fallbackBound)return;img.dataset.fallbackBound='1';img.addEventListener('error',()=>{img.classList.add('image-load-error');const art=img.closest('.product-art,.orbit-art,.shop-showcase-image');if(art&&img.dataset.managedImage==='true'){art.classList.remove('has-image');if(art.classList.contains('orbit-art'))art.style.backgroundImage='none';img.hidden=true;img.removeAttribute('src');img.classList.remove('image-ready');if(art.classList.contains('shop-showcase-image')&&!art.querySelector('span'))art.insertAdjacentHTML('beforeend','<span aria-hidden="true">IMAGE PENDING</span>');}},{once:true})})}
function navLabel(k){return getContent().siteText?.nav?.[k]||defaultContent.siteText.nav[k]}
function renderCommerceNavigation(){
  document.querySelectorAll('.bag-link').forEach(link=>link.remove());
  if(!siteConfig.commerceEnabled)return;
  document.querySelectorAll('.site-header').forEach(header=>{const isCart=location.pathname.toLowerCase().endsWith('cart.html'),key=isCart?'cart':'bag',link=document.createElement('a');link.className='bag-link';link.href='cart.html';link.setAttribute('aria-label',navLabel(key));link.innerHTML=`<span data-nav-label="${key}">${escapeHtml(navLabel(key))}</span> <span data-cart-count>0</span>`;header.append(link)});
}
...
function renderPublicContent(){const c=getContent(), text=c.siteText||defaultContent.siteText;
  ...
  if(pageKey){const p=c.pages[pageKey]||{};const heading=mainPage.querySelector('.page-heading');if(heading){const e=heading.querySelector('.eyebrow'),h=heading.querySelector('h1'),n=heading.querySelector('.heading-note');if(e){let ey=p.eyebrow||'';if(pageKey==='contact')ey=ey.replace(/[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}/g,'message channel');e.textContent=ey;}if(h)h.innerHTML=`${escapeHtml(p.titleLineOne||'')}<br><em>${escapeHtml(p.titleLineTwo||'')}</em>`;if(n){const note=String(p.note||''),vinted=pageKey==='shop'?note.match(/https:\/\/(?:www\.)?vinted\.[^\s]+/i):null,url=vinted?safeHttpsUrl(vinted[0]):'';n.textContent='';if(pageKey==='shop'&&url){const copy=note.replace(vinted[0],'').trim(),label=document.createElement('span'),link=document.createElement('a');if(copy){label.className='shop-note-copy';label.textContent=copy;n.append(label);}link.className='shop-vinted-link';link.href=url;link.target='_blank';link.rel='noopener noreferrer';link.textContent='Vinted ↗';n.append(link);}else n.textContent=note;}}}
...
function renderProducts(){
  ...
  const products=getContent().shop||[];
  if(!products.length){root.innerHTML='<div class="shop-empty" role="status"><strong>COMING SOON</strong><p>New objects from the Kosmik signal are being prepared.</p></div>';return;}
  if(!siteConfig.commerceEnabled){
    root.innerHTML=products.map((p,i)=>{const title=p.title||p.name||'',image=safeImageUrl(p.image);return `<article class="shop-showcase-card"><figure class="shop-showcase-image">${image?`<img loading="lazy" decoding="async" data-managed-image="true" src="${escapeHtml(image)}" alt="${escapeHtml(p.alt||title)}">`:`<span aria-hidden="true">${String(i+1).padStart(2,'0')} / IMAGE PENDING</span>`}</figure><div class="shop-showcase-copy"><h2>${escapeHtml(title)}</h2><p>${escapeHtml(p.description||'')}</p></div></article>`}).join('');return;
  }
...
function renderLinksAndLegal(){
  const legacyLabels={soundcloud:'SoundCloud',youtube:'YouTube',instagram:'Instagram',tiktok:'TikTok',facebook:'Facebook'},source=Array.isArray(siteConfig.links)?siteConfig.links:Object.entries(siteConfig.links||{}).map(([key,url])=>({label:legacyLabels[key]||key,url})),links=source.map(item=>({label:String(item?.label||'').trim(),url:safeHttpsUrl(item?.url)})).filter(item=>item.label&&item.url);
  document.querySelectorAll('[data-contact-socials]').forEach(nav=>{nav.replaceChildren(...links.map(item=>{const a=document.createElement('a');a.className='contact-social-link';a.href=item.url;a.target='_blank';a.rel='noopener noreferrer';a.textContent=`${item.label} ↗`;return a}))});
  const footer=document.querySelector('.site-footer');if(footer){const contacts=document.createElement('nav');contacts.className='footer-contacts';contacts.setAttribute('aria-label','Contact and social links');const email=String(siteConfig.businessEmail||'').trim();if(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){const a=document.createElement('a');a.href=`mailto:${email}`;a.textContent=email;contacts.append(a);}for(const item of links){const a=document.createElement('a');a.href=item.url;a.target='_blank';a.rel='noopener noreferrer';a.textContent=item.label;contacts.append(a);}if(contacts.childNodes.length)footer.append(contacts);const nav=document.createElement('nav');nav.className='footer-legal';nav.setAttribute('aria-label','Legal');const shopShowcase=!siteConfig.commerceEnabled&&location.pathname.toLowerCase().endsWith('shop.html');nav.innerHTML=(shopShowcase?['privacy','terms']:['privacy','terms','shipping','returns']).map(key=>`<a href="legal.html?page=${key}">${key[0].toUpperCase()+key.slice(1)}</a>`).join(' · ');footer.append(nav);}
  const legal=document.querySelector('[data-legal-content]');if(legal){let key=new URLSearchParams(location.search).get('page')||'privacy';if(!['privacy','terms','shipping','returns'].includes(key))key='privacy';document.querySelector('[data-legal-title]').textContent=key[0].toUpperCase()+key.slice(1);legal.textContent=getContent().legal?.[key]||'This information is being prepared. Use the contact form before placing an order.';}
}
function initMatrix(){
